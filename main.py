...
# Nova imagem de robô IA (domínio livre, robô azul)
img_url = "https://cdn.pixabay.com/photo/2017/01/31/13/14/robot-2027195_1280.png"
try:
...
with col1:
    if img:
        st.image(img, width=120, caption="Robô IA")
    else:
...
with col2:
    ...
    st.markdown(
        "<p style='color:gray;'>Aplicação interativa de IA para análise de dados tabulares.</p>",
        unsafe_allow_html=True
    )
...
try:
    ...
    st.sidebar.warning(
        "A chave da API do Google não foi encontrada nos segredos. Por favor, insira-a abaixo.")
    api_key_input = st.sidebar.text_input(
        "Chave da API do Google", type="password")
    if api_key_input:
        ...
        with st.chat_message("assistant"):
            with st.spinner("O agente está pensando..."):
                try:
                    ...
                    fig = plt.gcf()
                    has_plot = any(ax.has_data()
                                   for ax in fig.get_axes()) if fig else False

                    if has_plot:
                        st.pyplot(fig)
                        st.session_state.messages.append(
                            {"role": "assistant", "content": output_text, "figure": fig})
                    else:
                        plt.close(fig)
                        st.markdown(output_text)
                        st.session_state.messages.append(
                            {"role": "assistant", "content": output_text, "figure": None})

                except Exception as e:
                    error_message = f"Ocorreu um erro: {e}"
                    st.error(error_message)
                    st.session_state.messages.append(
                        {"role": "assistant", "content": error_message, "figure": None})
